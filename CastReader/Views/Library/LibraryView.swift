...
struct LibraryView: View {
    ...
    private var listView: some View {
        List {
            Section {
                ForEach(filtered) { rec in
                    Button { open(rec) } label: { HistoryRow(record: rec) }
                        .buttonStyle(.plain)
                        .disabled(opening)
                }
                .onDelete { offsets in
                ...
                Label("历史记录仅保存在本机，不会上传或同步到云端", systemImage: "lock.shield")
                    .font(.caption)
            }
        }
        ...
        .overlay {
            if opening {
                ProgressView().scaleEffect(1.2).padding(20)
    ...
    private func open(_ rec: HistoryRecord) {
        ...
        opening = true
        Task {
            let doc = await history.reopen(rec)
            opening = false
