...
final class CloudStorageCenter: ObservableObject {
    ...
    /// Resolves a possible crash between the provider-side commit and the
    /// account-scoped CloudConnectionStore commit.
    private func restoredPersistentProviderState(
        ...
        case .disconnected, .needsReauthorization:
            return provider == .googleDrive ? initialState : .disconnected
        }
