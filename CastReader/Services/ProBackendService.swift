...
actor ProBackendService {
    ...
    /// 查询 Pro/额度。userId 为 nil 时仅用 device_id。
    func fetchStatus(userId: String?) async -> ProStatusDTO? {
        ...
        do {
            ...
            return try JSONDecoder().decode(ProStatusDTO.self, from: data)
        } catch {
    ...
    func trackListen(seconds: Int, userId: String?) async {
