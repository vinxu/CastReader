...
assert.equal(sandbox.__crOfflineSourceRecover(3),true);
assert.equal(calls.at(-1),3,'an ignored turn re-seeks the exact uncommitted target');
navigation.readerState.isRendererLoading=true;
assert.equal(sandbox.__crOfflineSourceRecover(3),false,'do not restart an in-flight render');
navigation.readerState.isRendererLoading=false;
navigation.state.pagePositionRange={startPosition:3,endPosition:99};
assert.equal(sandbox.__crOfflineSourceRecover(3),false,'a ready target never advances again');
assert.equal(sandbox.__crOfflineSourceRecover(101),false,'source bounds still apply');
