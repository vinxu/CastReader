...
release_baseline=b6dd4d7
git -C "$integration_root" merge-base --is-ancestor "$release_baseline" HEAD || {
  printf '%s\n' 'Refusing to build: checkout does not contain the verified iOS 1.2.37 release.' >&2
  exit 1
}
cd "$integration_root"
...
git ls-files --others --exclude-standard CastReader CastReaderTests CastReaderUITests scripts/build-reader-more-integration.sh |
