...
> 历史记录：下述自适应策略已由同日的[固定 DeepFilter 发布](../fixed-deepfilter-rollout-20260903/REPORT.md)取代。当前两区新建音色统一使用 DeepFilter atten24，不再绕过、选择 atten100 或生成探针；旧 mode 文件设为 `off` 不能回滚。本文的 11/12 盲听结果仅属于旧自适应策略，不代表固定管线的胜率。
