...
最新完整链路进展见 [离线保存与阅读本地验收](offline-flow-acceptance-20260913.md)：包含正式入口、下载、离线书列表、阅读 UI、按需 OCR/TTS、断点与删除；真机最终验收按用户安排留待手机可用。

历史记录：2026-09-12 的单页 DEBUG 验证。2026-09-13 已推进为图片先保存、播放时本机 OCR：427 页约 2 分 19 秒保存完成，最新状态见 [图片缓存提速验收](image-only-acceptance-20260913.md)。此前逐页 OCR 的整书证据见 [旧整书验收记录](whole-book-acceptance-20260913.md)。下文阶段状态保留为当日历史，不代表当前实现。
