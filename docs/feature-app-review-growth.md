...
- 首次记录后至少 72 小时；
- 至少 2 个不同活跃日；
- 至少 3 个不同 read session，各自累计到真实播放 300 秒；
- 同一版本最多调用一次；
- 两次调用至少间隔 90 天；
- 滚动 365 天最多调用 3 次。

朗读时长只累计同一音频 segment 内 `0 < delta <= 2.01s` 的正向播放器 tick。跳读、回退和异常时间跳变不能制造评分资格。资格满足时仅写入 `pending`，不会在阅读器内立即打断用户。
...
- `store=app_store`
- 自动资格/请求：`trigger=third_five_minute_read`
