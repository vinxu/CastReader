...
### 4.1 `Services/OCRLayoutAnalyzer.swift` —— 递归 XY-Cut

纯几何、不依赖 Vision/UIKit、可离线单测。

#### 为什么是递归分解

**最初实现是固定两层（整页 → 切区 → 切栏），它错在层级结构**。报纸是**递归的矩形分割**：区域里还有区域，并列的两篇文章各自再分栏。固定层数表达不了这件事 —— 侧边那篇被裁切的文章会被当成主文的一栏，内容按层切碎后散插进主文的阅读流（实测主文开始前混进 25 行碎片）。

改用经典 **Recursive XY-Cut**（Nagy & Seth, 1984）：每次只在**最显著**的位置切一刀，再对两边递归，直到切不动。阅读顺序 = 深度优先遍历。
...
副产品：**「跨栏横幅」这个特例消失了** —— 标题与正文之间有横向空白，自然被横切成独立区域，不需要专门的 banner 逻辑。特例减少是结构变对的信号。
...
`depth`（贯穿度）是主项：贯穿全高的竖缝（分隔并列文章）因此能排在被跨栏标题打断的横缝之前。带宽用**平滑饱和**而非硬上限 —— `min(1.5, extent)` 会把「3.75 倍行高的区间隔」和「2.5 倍行高的栏缝」压成同一个数，谁先谁后退化成看数组顺序。同分时**横切优先**（文档自上而下流动）。

#### 两个轴必须对称

各自要剥掉会把自己那条缝填平的干扰项：竖切怕**通栏元素**（`columnarCandidates`），横切怕**贯穿全高的窄边栏**（`zoneCandidates`）。只做前者不做后者，顶层会一条横切都找不到。

`shoulder` 判据同理：竖切要求两侧都是实打实的正文，横切只要求两侧有行 —— **一个区的厚度可以只有一行**（大标题本身就是一个区）。

#### 通栏行提升到父层

竖切时跨越切割线的行**不按 midX 硬分给某一栏**。通栏元素不属于任何一栏，它在栏组的上方或下方，层级更高。硬分会让页脚黏在最后一栏尾巴上。

这同时消掉了一处自相矛盾：找栏缝时剥离通栏行、执行切割时却又把它们切进去。

#### 分解前的两步预处理

- **倾角矫正**：手持拍摄斜几度，栏就不再垂直，投影把栏缝整条抹平（实测直接 no-gaps）。±12° 内搜索使栏缝最清晰的角度，**只用于分析**，输出仍是原始行 id。
- **边栏拆分**：报纸边缘被裁切的文章贯穿全高，参与分解会被横切切碎。整块拆成独立区域排在主体之后。
...
**Android 已完成同等实现**（567 tests / 0 failures）：递归 XY-Cut、倾角矫正、方向推断、边栏拆分、通栏行提升、质量闸、选区裁剪与 Compose 框选。两个独立实现在同一份真实报纸几何（297 行）上得到相同结果。
