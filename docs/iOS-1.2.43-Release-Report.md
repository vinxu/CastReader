...
- 镜像恢复时显示 iPhone 正在使用，改为已授权正常 App 的 Xcode 真机测试通道。`testAuthorizedDeviceHomeWithoutAuthenticationBypass` 于 07:45 PASS，无登录绕过或夹具。新增 opt-in 中文克隆观察用例只修改 UI 测试源码，记录实际 UI 操作、截图和辅助功能树；其结果必须与设备生成/播放日志一起审查，不能单凭测试总数放行。正常构建签名已通过；完整用例随后在测试执行前遇到运行器 bootstrap 退出（Code 74），按原配置重试、关闭镜像独占 Xcode 后仍相同，三次均不计为核心 PASS。08:00 前正常 App 重启已回读中国 `pro=Y`；随后镜像恢复，08:21–08:35 完成下述真实交互验收。

- 08:21–08:35 中文克隆补验全部通过：曹操 `vl_b4bb75aa2d3e56fd90a2` 完整朗读 7 段；现有私人声音“中国区测试录制”实际 ID `vc_63a448ca35794f57b0aea0e189a68cb3`（不是原计划的另一个私人 ID）新生成并连续朗读，暂停/恢复后原位置继续，切回常规 `zf_001`。同一自有短文产生新的 5 块中文讲解，社区第 2→3 块、私人重播第 0→1→2→3 块均有真实生成、AVPlayer 完成与继续、解释字幕和原文标注；私人暂停后字幕/标注稳定，恢复后继续，再切回常规音色。实际音频进度由日志和镜像共同核对，未做录音式主观音质测评。
- 中文账本只读核验：社区 32 次成功生成共 186,456 ms，私人 29 次共 254,756 ms，合计 441,212 ms 与额度桶 used_ms 完全一致，reserved_ms 为 0；没有常规音色扣减行。界面依次显示 120 / 118 / 117 / 116 / 114 / 112 分钟。声音为 warm；不虚报线上冷启动。账本与设备记录保存在本版私有证据目录。
