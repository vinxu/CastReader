...
更新：2026-09-16。**先证明核心服务可用，再处理商店资料。** 正常流程只有五步；同一候选源码的有效结果可复用，不因继续任务而重跑。

使用 `$submit-castreader-ios-to-app-store`。完整“送审”指令授权到正式审核状态回读；仅修改 SOP、检查或打包不授权上传送审。

## 1. 一次确认基线与发布范围

- 读当前 `AGENTS.md`、最新发布记录及差异，选已验证发布工作树，记录 commit、应用源码/配置摘要、版本、Build。**不要默认从主目录旧分支打包。** 按该工作树要求运行基线检查。
