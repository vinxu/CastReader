...
## 2026-09-07 用户确认的截图与元数据规则

- 用户已创建 1.2.35，并删除展示旧 v1 订阅价格的付费页截图；该图今后停用，不自动补回。识别特征：标题 “Try Pro Free for 7 Days”、月付 $4.99、年付 $34.99；用户提供的参考文件为 `/Users/xuxuheng/Downloads/0x0ss.png`。这些是停用图的旧内容，不代表当前价格或试用政策。
- 本次 1.2.35 仅构建/上传、绑定新 Build、写入 11 语 What's New 并提交。保持用户当前版本中的名称、标题、副标题、描述、关键词、推广文字、链接与截图原样。
- 此类“仅更新说明”发布不能运行会同时写入 description/keywords/promotionalText 等字段的全量元数据同步器；`ASC_SKIP_APP_INFO=1` 仅跳过 App Info，仍不足以满足本次要求。只 PATCH 每个现有版本本地化的 `whatsNew`，提交前后比对其余文案与截图清单。
- 以后发版沿用用户当前维护的截图集合；历史模板或上一版截图不能覆盖用户的删除操作。新增或替换截图须来自后续明确任务。
- 本次用户版本 11 语 `promotionalText` 均为空，须保留；通用审核脚本把空推广文本当作错误，不能为了让脚本通过而补写。保留原始报告、仅对已确认的现状记录例外，再以 Apple 实际提交校验为准。本次该现状已成功送审；其他构建、语言覆盖、标题保护、截图与审核资料检查照常执行。
