...
## 本轮追加：iPhone / iPad Safari 模拟器验收

用户随后要求使用模拟器完整测试并适配 iPad。已在 iPhone 15 Pro Max 与 iPad Air 11-inch M4（均 iOS/iPadOS 26.5）用真实线上预设 TTS 验证：单次点击、逐词高亮、自动滚动、跨段续播、暂停/恢复、Heart/Bella 换音色、倍速、停止/重启、横竖屏与自然结束。

本轮修复：原生会话身份字段缺失（`769e1a0`）、iOS manifest 缺少 nativeMessaging、触摸首次点击被 hover 消耗，以及旋转后当前段落后续词落到屏幕外。原生合同 XCTest 已增加到 **12 项通过**；最终 Safari 资源门禁 **180 项**、Chrome/Safari 结构与合同对齐 **263 项**通过。

普通构建资源已同步进当前 iOS 工作树，保持 1.2.45（68），iPhone/iPad 使用同一 Safari 扩展目标；App 的设备家族为 `1,2`。只有专用验收包包含本地只读观测探针，交付资源与普通包均不包含。完整测试报告见 `docs/Safari-Simulator-Acceptance-2026-09-26.md`。

这些结果不替代最新版真机验收，也不覆盖全部第三方网站、真实付费/克隆音色、iPad 分屏和后台场景。首轮真机安装包的哈希是历史记录，不能视为本轮修复后的包。
